...
def create_ec2(num_instances, instance_type): 
    ...
    for i in range(num_instances):
        instances.append(EC2_RESOURCE.create_instances(
            ...
            MinCount=1,
            MaxCount=1,
            InstanceType=instance_type, # we will change accordingly (t2.nano is free for testing purposes)
            SecurityGroupIds=[security_group_id], # we use the security group id just created above.
            SubnetId=SN_ALL['Subnets'][i%N_SUBNETS]['SubnetId'], # we distribute instances evenly across all subnets
